...
import { agencyNotifications } from "./agency-notifications-data";

export function AgencyNotificationsPopover() {
  const [open, setOpen] = useState(false);
  const [notifications, setNotifications] = useState(
    agencyNotifications.map((notification) => ({ ...notification })),
  );
            ...
            <header className="flex items-center justify-between border-b border-black/7 px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-[#81867e]">
                  {unreadCount
                    ? `${unreadCount} unread agency updates`
