...
export function InboxList({
  ...
  onSelect: (conversation: AgencyConversation) => void;
}) {
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-black/8 bg-white">
      <header className="border-b border-black/7 p-4">
        ...
        <p className="mt-1 text-xs text-[#7b8078]">Agency conversations</p>
        <label className="mt-4 flex h-10 items-center gap-2 rounded-xl bg-[#f3f5f1] px-3">
          <Icon name="search" size={17} className="text-[#858a82]" />
          <input
            value={search}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center justify-between gap-2">
                  ...
                  <span className="shrink-0 text-[10px] text-[#8b9088]">
                    {conversation.lastMessageTime}
                  </span>
                </span>
                <span className="mt-0.5 block truncate text-[11px] font-medium text-[#687066]">
                  {conversation.company}
                </span>
                <span
                  className={`mt-1 block truncate text-xs ${
                    conversation.unread
                      ? "font-semibold text-[#383c37]"
                      : "text-[#81867e]"
                  }`}
                >
